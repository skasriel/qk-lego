...
export class OBJBrick extends Brick {
  ...
  save() {
    ...
    state = { ...state, brickType: OBJBrick.BrickType };
    return state;
  }
  ...
  new MTLLoader(manager).load(
    ...
    function (materials) {
      ...
      new OBJLoader(manager).setMaterials(materials).load(
        ...
        null,
        (error) => {
          console.log('Error downloading OBJ file ' + objFileName + ': ' + error);
    ...
    null,
    (error) => {
      console.log('Error downloading MTL file ' + mtlFileName + ': ' + error);
      errorsLoadingTemplates = true;
    }
  );
}
