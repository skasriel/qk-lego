...
const multX = 1.0,
  multY = 0.4,
  multZ = 1.0; // convert from Lego Units to Three.js units
...
function buildMeshFromBlock(block) {
  ...
  let cube = new THREE.BoxGeometry(
    block.length * multX,
    block.height * multY - brickSeparationY /*leave some room above*/,
    block.width * multZ
  );
  ...
  let rondRadius = 0.4;
  let rondHeight = 0.3; // hauteur des ronds en taille Three.js
  let cylinder = new THREE.CylinderBufferGeometry(
    rondRadius /*radius top*/,
    rondRadius /*radius bottom*/,
    rondHeight /*height*/
  );
...
function raycasterLookup() {
  if (selectMode)
    // don't update the selectedBlock if we've clicked on it.
    return;
  raycaster.setFromCamera(mouse, camera);
  let intersects = raycaster.intersectObjects(scene.children, true);
  if (intersects.length > 0) {
    if (selectedBlock != intersects[0].object) {
      if (selectedBlock) selectedBlock.material.emissive.setHex(selectedBlock.currentHex);
